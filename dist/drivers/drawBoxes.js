...
function drawBoxes() {
    ...
    GlobalVariables.gl.drawArrays(GlobalVariables.gl.TRIANGLES, 0, GlobalVariables.attribLength.box.vertex / 2);
}
function generateAndDrawBoxVertices() {
    let boxVertices = [];
    GlobalVariables.liveCells.forEach((point) => {
        const [x, y] = point.split(',').map(Number);
        let point1 = { x, y };
        let point2 = { x: point1.x + 1, y: point1.y };
        let point3 = { x: point1.x + 1, y: point1.y + 1 };
        let point4 = { x: point1.x, y: point1.y + 1 };
        ...
        boxVertices.push(...normalizedPt1, ...normalizedPt2, ...normalizedPt4, ...normalizedPt2, ...normalizedPt4, ...normalizedPt3);
    });
    GlobalVariables.gl.useProgram(GlobalVariables.program.box);
    GlobalVariables.attribLength.box.vertex = boxVertices.length;
    let float32vertex = new Float32Array(boxVertices);
    let vertexLocation = GlobalVariables.gl.getAttribLocation(GlobalVariables.program.box, 'vertex');
    if (GlobalVariables.vao.box == null) {
        ...
        updateVao(GlobalVariables.vao.box, [
            {
                bufferArray: float32vertex,
                type: GlobalVariables.gl.ARRAY_BUFFER,
                location: vertexLocation,
