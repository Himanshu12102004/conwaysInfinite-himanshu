...
class CanvasEvents {
    static addEvents() {
        ...
        GlobalVariables.canvas.addEventListener('mouseup', (e) => {
            ...
            const distance = Math.hypot(dragEndX - CanvasEvents.dragStartX, dragEndY - CanvasEvents.dragStartY);
            if (distance < 5) {
    ...
    static onResize(e = undefined) {
        ...
        if (GlobalVariables.screenDimensions.height < 500 ||
        ...
        let centerX = (GlobalVariables.bounds.maxX + GlobalVariables.bounds.minX) / 2;
        let centerY = (GlobalVariables.bounds.maxY + GlobalVariables.bounds.minY) / 2;
        ...
        if (GlobalVariables.showGrid) {
            ...
            let vertexLocation = GlobalVariables.gl.getAttribLocation(GlobalVariables.program.line, 'vertex');
            if (GlobalVariables.vao.line == null) {
                ...
                updateVao(GlobalVariables.vao.line, [
                    {
                        bufferArray: myAxes,
                        type: GlobalVariables.gl.ARRAY_BUFFER,
                        location: vertexLocation,
    ...
    static onclick(e) {
        ...
        let centerX = (GlobalVariables.bounds.maxX + GlobalVariables.bounds.minX) / 2;
        let centerY = (GlobalVariables.bounds.maxY + GlobalVariables.bounds.minY) / 2;
        let x = Math.floor((e.clientX * (rightBound - leftBound)) /
            GlobalVariables.screenDimensions.width +
            leftBound +
            centerX);
        let y = Math.floor(-((e.clientY * (topBound - bottomBound)) /
            GlobalVariables.screenDimensions.height +
            bottomBound) + centerY);
        if (GlobalVariables.liveCells.has(`${x},${y}`))
            GlobalVariables.liveCells.delete(`${x},${y}`);
        else
            GlobalVariables.liveCells.add(`${x},${y}`);
    }
...
CanvasEvents.isPanning = false;
CanvasEvents.isDragging = false;
CanvasEvents.dragStartX = 0;
CanvasEvents.dragStartY = 0;
export { CanvasEvents };
const blur = document.querySelector('.blur');
const obstructed = document.querySelector('.obstructed');
function obstruct(toObstruct) {
    if (toObstruct) {
