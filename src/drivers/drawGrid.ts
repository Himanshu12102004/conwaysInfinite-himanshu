...
function drawGrid() {
  ...
  GlobalVariables.gl.uniform3f(
    colorUniform,
    GlobalVariables.gridColor[0] / 255,
    GlobalVariables.gridColor[1] / 255,
    GlobalVariables.gridColor[2] / 255
  );
  GlobalVariables.gl.bindVertexArray(GlobalVariables.vao.line);
  GlobalVariables.gl.drawArrays(
    GlobalVariables.gl.LINES,
    0,
    GlobalVariables.attribLength.line.vertex / 2
  );
}
function generateGridVertices() {
  ...
  for (let i = bottomBoundY; i <= topBoundY; i++) {
    let y =
      (2 * (i - GlobalVariables.bounds.minY)) /
  ...
  for (let i = leftBoundX; i <= rightBoundX; i++) {
    let x =
      (2 * (i - GlobalVariables.bounds.minX)) /
