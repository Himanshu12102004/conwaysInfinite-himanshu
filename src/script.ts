...
import parsePattern from './drivers/parsePattern.js';
let speed = GlobalVariables.speed;
let speedController: any;
document.addEventListener('DOMContentLoaded', () => {
  ...
  (
    document.querySelector('.generation') as HTMLDivElement
  ).style.color = `rgb(${GlobalVariables.boxColor[0]},${GlobalVariables.boxColor[1]},${GlobalVariables.boxColor[2]})`;
...
function animate(timeStamp: number) {
  ...
  if (
    count % (GlobalVariables.maxSpeed - GlobalVariables.speed + 1) == 0 &&
    GlobalVariables.speed != 0
  ) {
    calcNextGen();
...
async function init() {
  ...
  colorFolder
    ...
    .onChange(() => {
      ...
      (
        document.querySelector('.madeBy') as HTMLDivElement
      ).style.color = `rgb(${GlobalVariables.boxColor[0]},${GlobalVariables.boxColor[1]},${GlobalVariables.boxColor[2]})`;
    });
  ...
  const vertexShaderBoxSource = await loadShader(
    './shaders/boxes/boxes.vs.glsl'
  );
  const fragmentShaderBoxSource = await loadShader(
    './shaders/boxes/boxes.fs.glsl'
  );
  GlobalVariables.shaders.box = compileShader(
    vertexShaderBoxSource,
    fragmentShaderBoxSource
  );
  GlobalVariables.program.box = createProgram(
    GlobalVariables.shaders.box.vertexShader!,
    GlobalVariables.shaders.box.fragmentShader!,
    GlobalVariables.gl
  );
...
let rules = document.querySelector('.rules') as HTMLDivElement;
function rulesElem() {
  if (rulesShown) {
...
function addListenerToRules() {
  const tempMap: Map<number, boolean[]> = new Map();
